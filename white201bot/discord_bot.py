...
import datetime
#? 이거 왜 뜸
TOKEN = 'Nzk3NzUzODAwOTE1OTQzNDU0.X_rEHw.u9LbbxBuuUwFnIUJISZwAdEELHs'
client = discord.Client()

@client.event
async def on_message(message):  #봇이 쓰는 명령에는 반응 안되게 하는 부분 저것도 cpd에서 해줘야함 흠.. 저런식으로 모듈을 다운받아야하는데 왜 wo
    ...
    if message.content.startswith("!강화"):
        ...
        exp = random.randrange(-5,15) #몇 레벨을 증가하고 감소할지 정하는 부분 이걸
        if exp >13:
